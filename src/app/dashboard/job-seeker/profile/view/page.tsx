...
const getMissingSections = (profile: ProfileData | null): string[] => {
  if (!profile) return ["Headline", "Location", "Resume or portfolio", "Skills"];

  const items: string[] = [];
  if (!profile.headline?.trim()) items.push("Headline");
  if (!profile.location?.trim()) items.push("Location");
  if (!profile.resumeUrl?.trim() && !profile.websiteUrl?.trim() && !profile.linkedinUrl?.trim()) {
    items.push("Resume or portfolio");
  }
  if (!profile.skills?.length) items.push("Skills");
  return items;
};
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-sm leading-7 text-sky-50/85 lg:text-base">
              Preview tone, clarity, and completeness before recruiters open your candidate story.
            </p>
          </div>
      ...
      <Card className="border-white/80 bg-white/94">
        <CardHeader className="border-b border-slate-100/80 pb-6">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-lg text-slate-600">{profile?.headline || "No headline yet"}</p>
            </div>
            {profile?.resumeUrl ? (
        ...
        <CardContent className="space-y-8 p-5 sm:p-6">
          {missingSections.length > 0 ? (
            <div className="rounded-[1.5rem] border border-amber-200/80 bg-amber-50/80 p-4">
              <p className="text-sm font-semibold text-amber-950">Recruiter-readiness note</p>
              <p className="mt-1 text-sm leading-6 text-amber-800">
                Add {missingSections.join(", ").toLowerCase()} to make this profile easier to scan and trust.
              </p>
            </div>
          ) : null}
          ...
          {profile?.experiences?.length ? (
            <section className="space-y-4">
              ...
              {profile.experiences.map((experience) => (
                <div
                  key={experience.id}
                  className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.18)]"
                >
            ...
            <section className="space-y-4">
              ...
              {profile.educations.map((education) => (
                ...
                >
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-600">{education.institution}</p>
